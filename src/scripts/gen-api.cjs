...
const SWAGGER_URL =
  ...
  "https://rwashnpreproduction.runasp.net/swagger/v1/swagger.json";
...
function generateEndpoints(swagger) {
  ...
  for (const tag of tags) {
    ...
    const items = byTag.get(tag).sort((a, b) =>
...
async function main() {
  ...
  execSync(
    `pnpm -s openapi-typescript "${SWAGGER_URL}" -o "${OPENAPI_TYPES_OUT}"`,
    { stdio: "inherit" }
  );

  // 2) Fetch swagger json for endpoints generation
  const swagger = isUrl(SWAGGER_URL)
    ? await fetchJson(SWAGGER_URL)
    : JSON.parse(fs.readFileSync(SWAGGER_URL, "utf8"));
