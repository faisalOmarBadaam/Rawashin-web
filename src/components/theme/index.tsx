...
const CustomThemeProvider = (props: Props) => {
  ...
  // Use systemMode for initial render to match SSR, then update after hydration
  const [currentMode, setCurrentMode] = useState<SystemMode>(systemMode)

  useEffect(() => {
    // After hydration, calculate the actual mode
    const actualMode = settings.mode === 'system'
      ? (isDark ? 'dark' : 'light')
