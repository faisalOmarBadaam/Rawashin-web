...
export default function EmployeesToolbar({ activeClientType, selectedClients, onRefresh }: Props) {
  ...
  const resolvedPathname = (() => {
    ...
    if (segments[segments.length - 1] === 'home') {
      segments[segments.length - 1] = 'employees'
  ...
  const addPath = `${resolveAddPath(resolvedPathname)}?type=${activeConfig.typeKey}`
